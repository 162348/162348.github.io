...
2. 代表的な関数の空間にはブラックボード・ボールド体を，測度の空間にはカリグラフィー体を用いる．
...
<!-- 
* **空集合** を次で表す：^[[@Shoenfield1967 p.243], [@新井敏康2011 p.2] の定め方に一致する．]
$$
\emptyset:=\{x\mid x\ne x\}.
$$
-->
...
<!-- 
* $A,B\subset X$ の [**差**](https://ja.wikipedia.org/wiki/%E5%B7%AE%E9%9B%86%E5%90%88) を次で表す：
$$
...
* [**非交和**](https://ja.wikipedia.org/wiki/%E9%9D%9E%E4%BA%A4%E5%92%8C) $A\sqcup B$ とは，$A\cap B=\emptyset$ が成り立つ際に，直和 $A\coprod B$ を $A\cup B$ と同一視したものをいう．^[[@斎藤毅-位相 p.10] は $A\coprod B$ と表す．[@伊藤清三63-Lebesgue積分] は $A+B$ と表す．]

* [**対称差**](https://ja.wikipedia.org/wiki/%E5%AF%BE%E7%A7%B0%E5%B7%AE) を次で表す：^[[@Dellacherie-Meyer1978] に一致．]
$$
...
* **自然数** をその集合論的構成の意味でも使う：^[[@vonNeumann1923] による定義である．[@斎藤毅-位相 pp.15-16], [Wikipedia](https://ja.wikipedia.org/wiki/自然数#形式的な定義) とも一致する．]
$$
...
* 正整数のなす部分集合を表すため，次の記法を用意する：^[[@Chopin+2022], [@Srinivasan2001] なども採用している．]
$$
[n]:=\{1,\cdots,n\}=n+1\setminus1.
$$

* $\R_+$ で **非負実数** の全体，^[[@Jacod-Protter2012], [@LeGall2016], [@鎌谷20-モンテカルロ p.106], [@Helemskii2006 p.2], [@Jacob2001] の記法に一致する．[@Evans2010 p.698] では同じ記法で正実数の全体を意味する．] $\R^+$ で **正実数** の全体がなす集合を表す．部分集合 $\bZ,\Q\subset\R$ や $\ov{\R}:=[-\infty,\infty]$ についても同様．特に $\N:=\bZ_+$．^[この運用は [@Jacod-Protter2012] に一致する．記法 $\N$ は [@Villani2009], [@Jacob2001] などでは正整数の全体 $\N=\Brace{1,2,3,\cdots}$ と定められている．[@Jacod-Shiryaev2003] も $\ov{\R}_+=[0,\infty]$ としている．]

<!--
* 実数 $x\in\R$ に対して，その整数部分を次で表す：^[[@Jacod-Protter2012] では $[x]$ で表される．]
$$
...
$$
\prod_\emptyset=1,\quad\sum_{\emptyset}=0,\quad0!=1.
$$
-->
...
<!-- 
* 写像 $f$ の [**値域**](https://ja.wikipedia.org/wiki/%E5%80%A4%E5%9F%9F) を $\Im f:=f(X)$ で表す．
...
* 部分集合 $A\subset X$ の **指標関数** といった場合は次を表す：^[[@Brezis2011 p.14] の使い分けに倣った．支持関数は英語で indicator function という [@Beck2017 p.14] 例2.1，[@寒野-土谷2014 p.110]，[@Ekeland-Temam1999 p.8]．]
  $$
...
* 写像 $I\ni i\mapsto X_i$ を **族** とも呼び，$(X_i)_{i\in I}$ と表す．しかしこの写像の値域も **族** と呼び，この場合は $\{X_i\}_{i\in I}:=\Im(X_i)_{i\in I}$ と表す．^[[@斎藤毅-位相 p.26] に倣った．この混用については p.35 で触れられている．これが集合をなすのは，ZF公理系のうちの置換公理による [@新井敏康2011 p.118]．] 

* 特に $I=\N$ のときは **列** とも呼ぶ．$I\fsub\N$ のときは組と同一視する．^[[@斎藤毅-位相 p.37] にも詳しく解説されている．このような態度は [concept with an attitude](https://ncatlab.org/nlab/show/concept+with+an+attitude) という．]
...
<!--
* 順序集合 $Y$ に値を取る関数 $f,g\in\Map(X,Y)$ について，$f\le g$ とは次の略記とする：
$$
\forall_{x\in X}\;f(x)\le g(x).
$$
-->

* 多くの場合，[一階の量化記号](https://ja.wikipedia.org/wiki/%E4%B8%80%E9%9A%8E%E8%BF%B0%E8%AA%9E%E8%AB%96%E7%90%86) $\forall$ を省略して次のような略記を用いる：
$$
...
* 関数 $g:\R\to\R$ に対して $O(g(x))\;(x\to x_0)$ とは，条件
$$
...
* 積空間 $\prod_{i\in I}X_i$ からの [**第 $i$ 射影**](https://ja.wikipedia.org/wiki/%E5%B0%84%E5%BD%B1_(%E9%9B%86%E5%90%88%E8%AB%96)) を $\pr_i$ で表す．^[[@Billingsley1999], [@Ethier-Kurtz1986], [@Jacob2001] などは $\pi_i$ で表す．]

* $x\in X$ での [**評価写像**](https://ncatlab.org/nlab/show/evaluation+map) を $\ev_x:Y^X\epi Y$ で表す．^[[@斎藤毅-位相 p.27] では値写像と訳している．]
...
* $\id_X$ で集合 $X$ 上の [**恒等写像**](https://ja.wikipedia.org/wiki/恒等写像) を表す：^[[@斎藤毅-位相 p.25], [@Jacob2001] に倣った．[@Villani2009] では $\mathrm{Id}$ で表す．]
$$
...
* $U\in\Op(X)$ を $U\osub X$ とも表す．閉集合 $F\csub X$ とコンパクト集合 $K\compsub X$ も同様の略記を用いる．

* [**$n$-単体**](https://ncatlab.org/nlab/show/simplex) を次で表す：
$$
...
* [束](https://ja.wikipedia.org/wiki/%E6%9D%9F_(%E6%9D%9F%E8%AB%96)) $L$ の元 $a,b$ に対して，上限と下限を次でも表す．^[[@DelMoral-Penev2014 p.xlvii], [@Bogachev2007 p.277] 4.1.(i) に一致する．[@Bogachev2007 p.277] では lattice を structure ともいう．]
  $$
  a\lor b:=\sup\{a,b\},\quad
  a\land b:=\inf\{a,b\}.
  $$
  例えば $\{\F_i\}_{i\in I}$ を 集合 $X$ の元がなす $\sigma$-代数の族とすると，これらの合併が生成する $\sigma$-代数を次で表す：^[[@Dellacherie-Meyer1978], [@伊藤清1991 p.137] に倣った．]
  $$
...
* 順序の入った位相空間の元の列 $(x_n)$ について，$x_n\nearrow f$ とは，収束 $x_n\to f$ だけでなく，列 $(x_n)$ が単調増加であることも含意する．^[[@Jacob2001] に一致．]
...
* 距離空間 $(T,d)$ の **開球** を $U_\ep(t)=U(t;\ep)$ で表す．^[[@斎藤毅-位相 p.75] に従った．[@Rudin-FunctionalAnalysis p.4], [@Jacob2001] では $B_r(t)$ で表す．]

* **閉球** を $B_\ep(t)=B(t;\ep)$ で表す．^[[@Pedersen1989 p.44], [@Evans2010 p.699] に倣った．] 特に **単位閉球** は $B:=B(0;1)$ で表し，$\R^n$ のものである場合は $B^n$ とも表す．^[[@Pedersen1989 p.41] など．] Banach空間 $X$ の双対空間 $X^*$ のものは $B^*$ とも表す．^[[@Pedersen1989 2.5.1 p.70] など．]

* 集合 $A\subset T$ と $\ep>0$ に対して，その $\ep$-開近傍を次で表す．^[[@Boucheron+2013] に倣った．ここでは $t$-blowup と呼んでいる．[@Gine-Nickl2021 p.27] では $d(x,A)\le\ep$ と定義しているが，我々は同じものを $\ov{A_\ep}$ で表すこととする．[@Dudley2002 p.393], [@Bogachev2007 p.192] では $A^\ep$ で表し，[@Dudley2002 p.407] は閉集合バージョンを $A^{\delta]}$ で表す．]
$$
A_\ep:=\Brace{x\in T\mid d(x,A)<\ep}.
$$


以降も，ある記号 $\F$ に関して $\F(x;y)$ と表される記法は， $\F_y(x)$ の別記法と捉える．^[すなわち， $\F(x;y)$ という記法は，$y$ は写像（あるいは関手） $\F$ のパラメータ付けをする添字として理解する数学的対象，$x$ は写像（あるいは関手）の引数として理解する数学的対象として峻別する態度がある．]


### 線型空間・ノルム・内積 {#subsec-linear-space}





* $\bF$-線型空間 $X$ の部分集合 $A,B\subset X$ と数 $\lambda\in\bF$ について，次の記法を用いる：^[Minkowski 和などとも呼ばれる．「[@Jacob2001] などが触れている．]
  $$
  \begin{align*}
      A&+B:=\Brace{a+b\in X\mid a\in A,b\in B},\\
  ...
  * 集合 $A\subset X$ が生成する部分空間を次で表す：^[[@斎藤毅-線型代数 p.33] に倣った．[@Jacob2001] などは $\operatorname{lin}(A)$ で表す．]
  $$
...
* 対角成分に $d_1,\cdots,d_n$ を持つ $n$-次正方行列を $\diag(d_1,\cdots,d_n)$ で表す：^[[@Evans2010 p.697] に一致する．]
$$
\diag(d_1,\cdots,d_n):=\begin{pmatrix}d_1&\cdots&0\\
\vdots&\ddots&\vdots\\0&\cdots&d_n\end{pmatrix}.
$$

* 行列 $A\in M_{mn}(\bF)$ の転置を $A^\top$ で表し，^[[@MacKay2003 p.599] に一致する．[@吉田朋広2006-数理統計] などは転置を $A'$ で表す．[@斎藤毅-位相 p.86] では ${}^t\!A$ と表す．[@Evans2010 p.697] は $A^T$．] 共役転置を $A^*$ で表す．^[随伴行列ともいう [@斎藤毅-位相 p.87]．] $\bF=\bR$ の場合は $A^\top=A^*$．

* 一般に内積を $(-|-)$ で表す．^[[@Pedersen1989 p.80] に倣った．[@Conway2007 p.2] では $\brac{x,y}$ で表されるが，[@Lang1995-Manifolds p.343] によるとこれは von Neumann の 1950 年代のセミナーでの記法であったという．]
...
* 任意の集合 $J$ に関して，$\R$ の Banach 空間としての [**$l^p$-直和**](https://ncatlab.org/nlab/show/direct+sum+of+Banach+spaces#definitions) を $l^p(J)$ で表し，ノルムを $\norm{-}_p$ で表す．^[[@Pedersen1989 p.50] に倣った．] $J=\N$ のとき，単に $l^p$ とも表す．
  ...
  * $\R^n$ の標準基底を $e_1,\cdots,e_n$ で表す．^[[@Evans2010 p.698] に一致．]
...
* 集合の族 $\cA\subset P(X)$ 上の関数 $\mu:\cA\to[0,\infty]$ に対して，[**外測度**](https://ja.wikipedia.org/wiki/%E5%A4%96%E6%B8%AC%E5%BA%A6) を次で定める：^[[@Bogachev2007 p.17] 定義1.5.1, [@Dudley2002 p.89] に倣った．[@vanderVaart-Wellner2023 p.6] では **外確率** という．]
$$
\begin{align*}
    \mu^*(A)&:=\inf\biggl\{\sum_{n=1}^\infty\mu(A_n)\in[0,\infty]\,\bigg|\,\{A_n\}\subset\A,A\subset\bigcup_{n=1}^\infty A_n\biggr\}.
\end{align*}
$$
...
* $\mu$-零集合と $\mu$-充満集合との全体がなす $\sigma$-代数を $2_\mu:=\sigma(\cN(\mu))$ で表す．^[[@伊藤清1991 p.137] に従った．]
...
* 測度空間の族 $(E_i,\cE_i,\mu_i)$ について，積集合 $\prod_{i\in I}E_i$ 上の [**積 $\sigma$-加法族**](https://ja.wikipedia.org/wiki/%E7%A9%8D%E6%B8%AC%E5%BA%A6) を次で表す：^[[@Bogachev2007 p.188], [@Lang1993 p.158] に従った．]
$$
\bigotimes_{i\in I}\cE_i=\sigma\paren{[\bigcup_{i\in I}]\pr_i^*(\cE_i)}.
$$


* この上の直積測度を $\bigotimes_{i\in I}\mu_i$ で表す．^[このような一般的な場合の定義は [@Bogachev2007 p.189] 参照．] $\abs{I}=n,\mu_i=\mu$ の場合は $\mu^{\otimes n}$ とも表す．

* 位相空間 $(X,\cO)$ 上の [**Borel $\sigma$-加法族**](https://ja.wikipedia.org/wiki/%E3%83%9C%E3%83%AC%E3%83%AB%E9%9B%86%E5%90%88) を $\B(X):=\sigma(\cO)$ で表す．
...
* 確率過程 $\{X_t\}_{t\in T}\subset\L(\Om;E)$ が積空間 $(E^T,\otimes_{t\in T}\cE)$ に定める写像 $X_-:\Om\to E^T$ を **転置** と呼ぶ．^[筆者が考案した名称．族 $(X_t)_{t\in T}:T\to\L(\Om)$ としての見方と転置の関係になっているところから．[@伊藤清1991 p.232] は **見本過程（関数）** と呼び，記法 $X_\bullet$ を採用している．[@Baudoin2014-DiffusionProcess p.9] は application，[@Metivier1982 p.4] は the random function of the process と呼んでいる．]
...
\Delta x(t):=x(t)-x(t-)
$$
で表す．^[[@Jacod-Shiryaev2003 p.325], [@Protter2005 p.25] に倣った．] ただし，$x(0-)=x(0)$ と約束する．^[[@Jacod-Shiryaev2003 p.3] に倣った．この結果，$\Delta x(0)=0$ であることに注意．]

* 確率空間 $(\Om,\F,\P)$ 上の [**情報系**](https://ja.wikipedia.org/wiki/%E6%83%85%E5%A0%B1%E7%B3%BB) $(\F_t)_{t\in\R_+}$ とは，右連続性
$$
\F_t=\F_{t+}:=\bigcap_{s>t}\F_s
$$
を満たす増大系（フィルトレーション） $\F_s\subset\F_t\;(s\le t)$ をいう．^[フィルトレーションと言ったときに右連続性も課すのは [@Jacod-Shiryaev2003], [@Protter2005] に倣った．記法は [@伊藤清1991 p.239] に倣った．] これを $\bF:=(\F_t)$ などとも表す．

* フィルトレーションの左極限を次のようにも表す：^[[@Jacod-Shiryaev2003 p.2] 定義1.2, [@Bass2011 p.1], [@Dellacherie-Meyer1978 p.114], [@Revuz-Yor1999 p.42], [@Metivier1982 p.3] に倣った．]
$$
\F_{t-}:=\bigvee_{s<t}\F_s,\quad(t\in\ov{\R}_+),
$$

* 確率空間 $(\Om,\F,\P)$ とその上のフィルトレーション $\bF:=(\F_t)_{t\in\R_+}$ からなる 4-組 $(\Om,\F,\bF,\P)$ を [**確率基底**](https://encyclopediaofmath.org/wiki/Stochastic_basis) という．

* 確率基底 $(\Om,\F,\bF,\P)$ の情報系 $(\cF_t)$ が **完備** であるとは，$\cN(\P)\subset\F_0$ を満たすことをいう．このとき確率空間 $(\Om,\cF,\P)$ も完備であることが必要であることに注意．確率基底が完備な情報系を備えることを **通常の条件** を満たすとも言う．^[[@Metivier1982 p.3] [@Protter2005 p.3] [@Dellacherie-Meyer1978 p.114] など参照．]

* 過程 $\{X_t\}_{t\in\R}$ がフィルトレーション $(\F_t)$ に **適合的** であるとは，次を満たすことをいう：
$$
X_t\in\F_t,\quad(t\in\R).
$$

* $T\times\Om$ 上の 可予測／随意／発展的可測／可測／適合 $\sigma$-代数を $\cP,\cO,\cA,\cM,\cA_0$ で表す．^[[@Metivier1982 p.4] では適合過程，発展的可測過程を定める $\sigma$-代数を $\cM_0,\cM_1$ としている．さらに [@Metivier1982 p.11] では可予測矩形を $\cR$，これが生成する（有限）集合体を $\cA$ としている．]
...
* ある部分集合 $I\subset\R_+$ に値を取る停止時の全体を $\bT_I$，そのうち特に値域が有限部分集合になるものの部分集合を $\bT_I^f$ で表す．^[[@Metivier1982 p.80] 定義13.1．]

* $\R_+$ で添字付けられた実マルチンゲールのうち，
  * 一様可積分なものを $\bM$ で表す．^[[@Jacod-Shiryaev2003 p.11] 1.40 では $\cM$ で表される．]
  * $L^p(\P)$-可積分なものを $\bM^p$ で表す．
  * $L^p(\P)$-有界なものを $\bM^p_\infty$ で表す．$p=2$ の場合の元を特に $L^2$-有界マルチンゲールと呼ぶ．^[[@Jacod-Shiryaev2003 p.12 定義1.41] では $\cH^2$ で表され，square-integrable martingale と呼ばれる．]
  * 上限が $L^p(\P)$-可積分なものを $\bH^p$ で表す．
  * $0$ から始まる局所マルチンゲールの全体を $\bL$ で表す．^[[@Jacod-Shiryaev2003 p.43] 4.20 では $\cL$ で表される．]

* 過程 $A$ が有界変動であるとは，任意の閉区間 $[0,t]$ 上で見本道が殆ど確実に有界変動になることをいう．^[[@Jacod-Shiryaev2003 p.31 定義I.3.1], [@Metivier1982 p.94] に一致．] その全体を $\bW^0$ で表す．^[[@Metivier1982 p.149] では $\cW^0$ で表す．[@Jacod-Shiryaev2003 p.27 I.3.1] では $\cV$ で有界変動かつ適合$D$-過程で $A_0=0$ を満たすものの全体を表す．]
  * このときの変動の過程を $\Var[A]$ で表す．^[[@Jacod-Shiryaev2003 p.27] は $\Var[A]$ で，[@Metivier1981 p.149] は $\abs{A}$ で表す．]
  * さらに $\{\Var[A]_t\}\subset\cL^p(\Om)$ を満たすもののなす部分空間を $\bW^p$ で表す．
  * さらに可積分変動を持つ過程 $\E[\Var[A]_\infty]<\infty$ のなす部分空間を $\bW^p_\infty$ で表す．
  * 特に単調増加過程の全体を $\bW_+^0$ で表す．

* $D$-修正を持つクラス(LD)の擬似マルチンゲールの全体を $\bQ$，劣マルチンゲールの全体を $\bQ_+$ で表す．^[[@Metivier1981 p.149] に倣った．] このとき，$\bQ_\loc$ が特別な半マルチンゲールの全体を表す．
