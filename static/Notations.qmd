...
* 確率過程 $\{X_t\}_{t\in T}\subset\L(\Om;E)$ が積空間 $E^T$ に定める写像 $X_-:\Om\to E^T$ を **転置** と呼ぶ．^[筆者が考案した名称．族 $(X_t)_{t\in T}:T\to\L(\Om)$ としての見方と転置の関係になっているところから．[@伊藤清1991 p.232] は **見本過程（関数）** と呼び，記法 $X_\bullet$ を採用している．[@Baudoin2014-DiffusionProcess p.9] は application，[@Metivier1982 p.4] は the random function of the process と呼んでいる．]
...
$$
を満たす増大系 $\F_s\subset\F_t\;(s\le t)$ をいう．^[フィルトレーションと言ったときに右連続性も課すのは [@Jacod-Shiryaev2003], [@Protter2005] に倣った．記法は [@伊藤清1991 p.239] に倣った．] これを $\bF:=(\F_t)$ などとも表す．
...
* 情報系 $(\cF_t)$ が **完備** であるとは，$\cN(\P)\subset\F_0$ を満たすことをいう．このとき確率空間 $(\Om,\cF,\P)$ も完備であることが必要であることに注意．

* 確率空間 $(\Om,\F,\P)$ とその上のフィルトレーション $(\F_t)_{t\in\R_+}$ からなる 4-組 $(\Om,\F,(\F_t),\P)$ を [**確率基底**](https://encyclopediaofmath.org/wiki/Stochastic_basis) という．確率基底が完備な情報系を備えることを **通常の条件** を満たすとも言う．^[[@Metivier1982 p.3] [@Protter2005 p.3] [@Dellacherie-Meyer1978 p.114] など参照．]
...
* $T\times\Om$ 上の 可予測／随意／発展的可測／可測／適合 $\sigma$-代数を $\cP,\cO,\cA,\cM,\cA_0$ で表す．
