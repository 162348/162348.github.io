...
[![2.3 Banach 空間](Files/Notations/norm_X_p.svg)](#subsec-Banach-spaces)
...
Here, we define all mathematical concepts as sets under [ZFC axioms](https://en.wikipedia.org/wiki/Zermelo%E2%80%93Fraenkel_set_theory#Axioms).^[The category of sets $\Set$ is very suitable for the foundation of mathematics [nLab](https://ncatlab.org/nlab/show/Set#properties).] The symbol $:=$ is a shorthand for the statement "the right-hand side defines the left-hand side, and the equality holds as a result".^[[@DelMoral-Penev2014], [@Helemskii2006], [@MacKay2003 p.600] agree with this definition. $\equiv,\overset{\text{def}}{=}$ are also commonly used. [@Crisan-Doucet2002], [@Smith2010] use $\overset{\triangle}{=}$. Here, we avoid these symmetric symbols. Some people also use $=_{\text{df}}$ [@Quine-Szczotka1994].]
...
### Banach 空間 {#subsec-Banach-spaces}
...
In this section, $(E,\cE)$ is always a [measurable space](https://ncatlab.org/nlab/show/measurable+space).^[[@Revuz1984]，[@LeGall2016], [@DelMoral04-FeynmanKacFormulae] denote an arbitrary measurable space by $(E,\cE)$.]
...
### 確率分布 {#subsec-distributions}



* The set of all **probability measures** on $(E,\cE)$ is denoted by $\cP(E,\cE)$. 

* When $E$ is a topological space
  * an abbreviation $\cP(E):=\cP(E,\cB(E))$ is used for the set of **Borel probability measures**.^[[@Jacod-Shiryaev2003 p.347], [@Crisan-Doucet2002], [@Ethier-Kurtz1986 p.96], [@Bogachev2007 p.228] agree with this definition. [@Kechris1995 p.109], [@Villani2009] use italic $P(E)$ for this set.]

  * Among them, the set of [**Radon probability measures**](https://ja.wikipedia.org/wiki/%E3%83%A9%E3%83%89%E3%83%B3%E6%B8%AC%E5%BA%A6) on $(E,\cB(E))$ is denoted by^[This notation is derived from [@Pedersen1989 p.72]. [Section 7.2 @Bogachev2007 p.76] denotes $\cP_r(X)$ for this set. Radon measure is a Borel measure satisfying the following internal regularity, or tightness, condition: $$\forall_{B\in\B(E)}\;\forall_{\ep>0}\;\exists_{K\compsub B}\;\mu(B\setminus K)<\ep$$ [@Bogachev2007 pp.68-69] Definition 7.1.1, 7.1.4.]
  $$
  P(E)\subset\cP(E).
  $$
...
* When $(E,d)$ is a metric space, the **$p$-th Wasserstein space** is denoted by
  $$
  \cP_p(E):=\cP_p(E,d):=\Brace{\mu\in\cP(E)\,\middle|\,\exists_{x_0\in E}\;\int_Ed(x_0,x)^p\mu(dx)<\infty}.
  $$

* The set of [**couplings**](https://en.wikipedia.org/wiki/Coupling_(probability)) between probability measures $\mu,\nu\in\cP(E)$ is denoted by^[Note that a coupling is a Radon measure, following [Section 8.10(viii) @Bogachev2007 p.235] and [Remark 6.5 @Villani2009 p.95]. [@Kulik2018] uses $\cC$ for this set. [@Bogachev2018 p.105], [@Villani2009 p.XXI] use $\Pi(\mu,\nu)$, [@Ethier-Kurtz1986 p.96] uses $\cM(\mu,\nu)$, [@Dudley2002 p.420] 11.8节 uses $M(\mu,\nu)$, and [@Figalli-Glaudo2023] uses $\Gamma(\mu,\nu)$.]
  $$
  C(\mu,\nu):=\Brace{\pi\in P(E^2)\:\middle|\:\substack{(\pr_1)_*\pi=\mu,\\(\pr_2)_*\pi=\nu.}}
  $$


* $\rN_d(\mu,\Sigma)\in P(\R^d)$ is the $d$-dimensional [**normal distribution**](https://ja.wikipedia.org/wiki/%E6%AD%A3%E8%A6%8F%E5%88%86%E5%B8%83)．^[[@竹村彰道2020] agree with this notation.] Its density is represented by
    $$
    \phi_d(x;\mu,\Sigma):=\dd{\rN_d(\mu,\Sigma)}{\ell_d}(x).
    $$

* $\rU(A)\in P(\R^d)$ is the [**uniform distribution**](https://ja.wikipedia.org/wiki/%E9%80%A3%E7%B6%9A%E4%B8%80%E6%A7%98%E5%88%86%E5%B8%83) on the set $A\subset\R^d$．

* The [**Delta measure**](https://ja.wikipedia.org/wiki/%E3%83%87%E3%82%A3%E3%83%A9%E3%83%83%E3%82%AF%E6%B8%AC%E5%BA%A6) on the point $x\in E$ is denoted by $\delta_x$．^[Also called Dirac measure. [@Jacod-Shiryaev2003 p.68], [@Protter2005 p.299], [@Jacob2001] use $\ep_x$．[@Protter2005 p.299] uses $\delta_x$ for the Dirac function.]

* The [**distribution function**](https://ja.wikipedia.org/wiki/%E7%B4%AF%E7%A9%8D%E5%88%86%E5%B8%83%E9%96%A2%E6%95%B0) of a random variable $X\sim\nu\in P(\R^d)$ is denoted by
  $$
  F_X(a):=F_\nu(a):=\P[X_1\le a_1,\cdots,X_d\le a_d],\quad(a=a_{1:d}\in\R^d).
  $$

* When $d=1$, the [**generalized inverse**](https://en.wikipedia.org/wiki/Quantile_function) of the distribution function is denoted by^[[@Gerber+2019] agrees with this notation. [@竹村彰道2020 p.16] calls it 分位点関数 (quantile function)，[@森口繁一-確率表現関数] calls it 確率表現関数 (probability representation function). [@Dudley2002 p.283] denotes it $X_F$．]
  $$
  F^-_\nu(u):=\inf\Brace{x\in\R\mid F_\nu(x)\ge u},\quad(u\in(0,1)).
  $$
...
### Generalized Functions {#subsec-generalized-functions}
...
### Stochastic Analysis {#subsec-stochastic-analysis}
