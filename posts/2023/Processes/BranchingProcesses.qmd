...
1. $(Y_k)_{k=1}^\infty$ が独立であるとき，$(X_k)_{k=0}^\infty$ も独立である．これより，任意の時点 $m_1<\cdots<m_k<n\in\N$ について，$T_{m_k-1}$ が与えられた下で，$X_k\;(k\ge m_k)$ と $\F_{m_k-1}$ とは独立である．よって特に，$(X_{m_k},T_{m_k-1})$ が与えられた下で，$(X_n,T_{n-1})$ と $(X_{m_i},T_{m_i-1})\;(i<k)$ とは独立である．よって，[条件付き独立性の性質](../Probability/条件付き期待値の問題.qmd#sec-conditional-independence) より
$$
\begin{align*}
