...
5. 条件付き確率は $\P[Y\in B|X]:=\E[1_{\Brace{Y\in B}}|X]$ と定義する．
:::
...
[@Dudley02-RealAnalysisAndProbability 10.1節 p.336], [@吉田朋広2006-数理統計 p.43] がおすすめな参照先．[@舟木-確率論 p.88] が入門しやすい．$X\in L^2(\Om)$ でいい場合は，より「射影」としてわかりやすい特徴付けがある（ @sec-characterization ）．これのおすすめは [@Jacod-Protter2004 第23節 p.200], [@Kallenberg21-Foundations p.164]．
...
$$
を与えた下での条件付き期待値 $\E[Y|\sigma(X)]$ と，次のように関係する．^[[@Dudley02-RealAnalysisAndProbability p.340] など．] $\E[Y|\sigma(X)]$ は定義 @sec-def 1から $\sigma[X]$-可測であるが，可測性の特徴付け（後述）から，これはあるBorel可測関数 $f$ について，$$\E[Y|X]=f(X)\;\as$$
...
[@Dudley02-RealAnalysisAndProbability 定理4.2.8 p.128] は $S=\R$ の場合，[@Landkov72-Probability] は $S=\R^m$ の場合, [@Kallenberg21-Foundations 補題1.14 p.18] に一般の標準Borel空間の場合の証明がある．[nLab](https://ncatlab.org/nlab/show/conditional+expectation) も極めて参考になる．
...
だが，$\cG$ がある完備可分距離空間に値を取る確率変数 $Y$ について $\cG=\sigma(Y)$ である場合など，殆どの場合で，うまく $\P$ を取ることが出来る．^[[@Dudley02-RealAnalysisAndProbability 定理10.2.2 p.345]．一般には Borel空間に値を取る確率変数について成り立つ [@Kallenberg21-Foundations p.165]．] このように，a.s. 抜きで正式に確率測度として定まる場合，その確率核 $\P:E\times\cG\to[0,1]$ を，**正則**条件付き確率と呼び分ける．
