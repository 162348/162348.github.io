...
はモデル $p(\theta),p(\y|\theta)$ の設定をいくら簡単にしても根本的に計算が困難で実行不可能なのである．これを解決する分野を**ベイズ計算**という．
:::

## ベイズによる「ベイズ計算」

上述の理由により，Bayesが興味があった値 $\E[1_{(a,b)}(\theta)|\y]$ を実際に計算する方法が，論文 [@Bayes1763] の大部分を占めることになる．

まず，男児の数
$$m:=\#\Brace{i\in[n]\mid y_i=1}$$
が非常に多いか，非常に少ない場合については，被積分関数を二項展開して項別積分により計算することを提案した．しかしこの手法は明らかに今回の例では使えず（我々は男女の出生率にそこまで偏りがないことを経験的に知っている），真に興味のある場合を包含していない．

この真に興味のある一般的な場合については，Bayesは $\E[g(\theta)|\y]$ の値を上下から評価するにとどまった．

そのこともあってか，論文 [@Bayes1763] は実はBayesの死後にRichard Priceによって投稿されたものであり，生前に自ら投稿・発表した訳ではなかった．^[なお，[@Stigler1990] によると実際の出版年は1764年であったという．] 当然，発表当時も全く注目を受けなかった．

::: {.callout-important title="ベイズ計算の興り" icon="false"}
...
上述の「ベイズ統計学は理念的に好ましかろうと，実際に実行不可能である」という問題は，本質的には計算機のみによって解決されることになり， [@Bayes1763] のアイデアが真に日の目を見るには2世紀以上待たなければならない．

この2世紀の間，ベイズ統計学はそのポテンシャルが評価されているだけで，本質的な解決は誰もできず，真に進展したのは計算機の発明からも時間が経ち，PCも普及したここ30年のことである．
