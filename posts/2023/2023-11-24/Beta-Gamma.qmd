...
$C^1$-微分同相 $T:A\iso B$ に対して，^[記法 $\iso$ は [記法一覧](../2023-12-12/Notations.qmd#sec-maps) 参照．] $B$ 上の分布 $\pi\in\cP(B)$ の $T$ による引き戻し $T^*\pi$ の密度 $p^*$ が，$\pi$ の密度 $p$ と Jacobian の積になる：
$$
p^*(x)=p(T(x))\abs{J_T(x)}\quad(x\in A).
$$
