...
[教科書](../../../static/Lectures.qmd#sec-法律家のための統計数理) [@草野2016] では「効用」という概念を金銭的利得と関連付けて説明しているが，そのためにわかりにくい提示の仕方になっている．^[加えて，p.55 に「上記の公理からの演繹的推論によって次の結論が導き出される」とあるが，これは「効用」「単調増加」などの未定義用語を含むため，「演繹的推論によって」というのは誤謬というべきである．]
...
$$
3. （独立性）任意の $x,y,z\in X$ に対して，$x\succsim y$ ならば，任意の $\al\in(0,1)$ について
$$
\al x+(1-\al)z\succsim\al y+(1-\al)z.
$$
4. （アルキメデス性）^[[教科書](../../../static/Lectures.qmd#sec-法律家のための統計数理) [@草野2016] では「連続性」とあったが，ここでは [@vonNeumann-Morgenstern1944] の原著に従う．] 任意の $x,y,z\in X$ に対して，$x\succsim y\succsim z$ ならば，ある $\al,\beta\in(0,1)$ が存在して，
$$
...
$X$ を [Polish 空間](https://ja.wikipedia.org/wiki/%E3%83%9D%E3%83%BC%E3%83%A9%E3%83%B3%E3%83%89%E7%A9%BA%E9%96%93)，$\precsim\;\subset P(X)^2$ を連続で独立な選好関係とする．^[記法については [数学記法一覧](../2023-12-12/Notations.qmd#sec-distributions) 参照．] このとき，ある有界連続関数 $u\in C_b(\R)$ が存在して，
$$
...
[教科書](../../../static/Lectures.qmd#sec-法律家のための統計数理) [@草野2016] にいう「期待効用定理」とはこの定理を指すものと思われる．この $u\in C_b(\R)$ を **効用関数** (utility function) という．

問題2-2で用いていた金銭的利得の空間とは $X=\R_+$ の場合であり，これは定理の条件を満たす．よって任意の連続で独立な選好関係 $\succsim$ を用意することで，金銭的利得の代わりに効用 $u$ の期待値 $U$ を用いれば，特定のリスク選好性に対応して，同様の議論を用いて意思決定分析が可能になる．
