...
::: {.callout-important}
## まとめ
1. 符号付き測度^[ここで符号付き測度とは，$\sigma$-加法的な集合関数 $\mu:\Sigma\to[-\infty,\infty]$ であって，値域には $\infty,-\infty$ のいずれか一方しか含まれないもの，としている．[数学記法一覧](../../Surveys/Notations.qmd#sec-measures) も参照．] $\mu:\Sigma\to[-\infty,\infty]$ は，有限（＝$\R$-値）ならば有界であり（ @sec-1 ），有界ならば $\R$-値である．
2. 上に有限ならば上に有界である，というような結果も直接示せる（ @sec-2 ）．
3. $\sigma$-加法性まではいかず，有限加法性しか満たさない場合，$\R$-値でも有界とは限らない．^[[@Giesy70-FiniteValuedFinitelyAdditiveUnboundedMeasure]]
4. そもそも一般の有限次元Banach空間値測度 $\mu:\Sigma\to B$ は自動的に有界になる（ @sec-3 ）．しかし，$B$ が無限次元の場合は有界とは限らない．^[Birkhoff1935]
:::
...
証明は [@吉田耕作91-現代解析入門] はHahn分解を先に，Jordan分解を後に与えている．[@Bogachev2020 p.72] 定理2.6.1 も同様だが，$\R$-値の場合に限って証明を与えている．一方で [@Dunford-Schwartz1] は有限加法的測度のJordan分解を先に与えている．
...
$$
\nu_n(A):=\frac{1}{n}\ell(A\cap[n-1,n]),\qquad n\in\N^+,A\in\B(\R_+)_{\ell}
$$
と定め，$\nu(A)_n:=\nu_n(A)$ とすると，確かに $\nu(A)\in l^2$：
$$
\begin{align*}
