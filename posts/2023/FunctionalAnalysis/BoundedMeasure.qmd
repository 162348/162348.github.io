...
この設定の下で，$B=[-\infty,\infty]$ のときに有限（＝$\R$-値）であること，有界であること，有界変動であることの３概念がどのように関係するかを見る．

::: {.callout-important}
## まとめ
1. 符号付き測度^[ここで符号付き測度とは，$\sigma$-加法的な集合関数 $\mu:\Sigma\to[-\infty,\infty]$ であって，値域には $\infty,-\infty$ のいずれか一方しか含まれないもの，としている．[数学記法一覧](../../Surveys/Notations.qmd#sec-measures) も参照．] $\mu:\Sigma\to[-\infty,\infty]$ は，有限ならば有界であり（ @sec-1 ），有界ならば有限である．
2. 符号付き測度 $\mu$ が有界であることと，有界変動であることは同値である．$B$ が有限次元Banach空間である場合でも同値である．
...
5. $B$ が有限次元Banach空間であるとき $\mu:\Sigma\to B$ は自動的に有界変動になり（ @sec-3 ），特に有界である．しかし，$B$ が無限次元の場合は有界変動とは限らない^[Birkhoff1935] が，有界ではある．^[[@Dunford-Schwartz1958 p.127] III.4.5]
:::
...
::: {.callout-tip icon="false"}
## 系（Jordan分解）^[[@Dunford-Schwartz1959 p.130] では，有限なJordan測度に関連する分解をJordan分解，有限とは限らない測度に関する分解をHahn分解と呼び分けているが，現代では空間の分解をHahn，測度の分解をJordanと呼び分けることが主流であるようである．[@Halmos1950] も後者．]
...
::: {.callout-tip icon="false"}
## 例（全変動）^[(1), (3)は [@Dunford-Schwartz1958 p.97]．(2)は [@吉田耕作1991 p.389] 例7.1，[@Rudin1987 p.125] 6.13．]
$(S,\Sigma,\nu)$ を測度空間とする．
...
## 証明
^[[@Dunford-Schwartz1958 p.127] 補題III.4.4 参照．]
...
::: {.callout-note icon="false"}
## 証明
^[[@Lang1993 p.198]. [@Birkhoff1935] も参照．]
