...
* スケーリング解析の導入＋既存研究 ([-@sec-scaling] -- [-@sec-global-refresh])
* 今回の研究成果 ([-@sec-comparison] -- [-@sec-experiment])
...
### [BPS]{.color-blue} $\textcolor{#0096FF}{\{X_t\}}$ のスケーリング解析 {#sec-scaling}
...
実際の分布では $\sigma^2$ の値はわからない．これを推定するのが収束判定．
...
## 引用文献 {.unnumbered .unlisted visibility="uncounted"}
