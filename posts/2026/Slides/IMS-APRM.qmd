---
title: "High-Dimensional Behaviour of Piecewise Deterministic Monte Carlo"
...
Let me start by placing PDMP in the broader history of Monte Carlo methods.
The history starts with a Markov chain driven by a simple uniform random variable, originally simulated on the MANIAC computer [@Metropolis+1953]. Then the attention naturally shifted to more structured stochastic processes, such as Langevin diffusion [@Rossky+1978] and PDMP [@Michel+2014], [@Peters-deWith2012].
:::
...
::: {.content-visible when-format="html" unless-format="revealjs"}
PDMP is a class of Markov processes that complements the diffusion process.
A PDMP is a `diffusion' process driven by jumps, rather than a Brownian motion.
An important consequence is a strong ballistic / transport property, which is typically absent in a Langevin diffusion. Informally, the Brownian term moves only on the scale of $\Delta B_t \sim \sqrt{\Delta t}$.
:::

### Markov Chain Monte Carlo

![](IMS-APRM/BPS_LMC_U_ergodic_average.gif){fig-align="center"}

::: {.content-visible when-format="html" unless-format="revealjs"}
These processes are routinely used in the MCMC methodologies.
However, notice that the trajectory of Langevin diffusion requires a discretization, while the output of PDMP potentially allows sparse representations, by retaining the event-time information and the new velocity at these points.
:::

### Classical Methods Turn Out to Be [PDMPs]{.color-unite} in Disguise
...
![](IMS-APRM/BPS_100d_Potential_wide.svg){fig-align="center"}
