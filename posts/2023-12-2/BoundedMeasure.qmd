...
## まとめ
* 符号付き測度^[ここで符号付き測度とは，$\sigma$-加法的な集合関数 $\mu:\Sigma\to[-\infty,\infty]$ であって，値域には $\infty,-\infty$ のいずれか一方しか含まれないもの，としている．] $\mu:\Sigma\to[-\infty,\infty]$ は，有限（＝$\R$-値）ならば有界であり，有界ならば $\R$-値である．
* 同じことは一般の有限次元Banach空間値測度 $\mu:\Sigma\to B$ でそのまま成り立ち，$B$ が無限次元の場合，$\R$-値でも有界とは限らない．^[[@Lang1993-Analysis p.198]]
