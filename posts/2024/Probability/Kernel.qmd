...
可測空間 $E$ 上の確率核 $P$ が定まると，任意のスタート地点 $x\in E$ に対して，これを起点とする Markov 過程 $\{X^x_n\}_{n=0}^\infty\subset\L(\Om)$ が構成できる．

ここで謎の標本空間 $(\Om,\F,\P)$ が出てきたし，$X^x_n$ は確率変数であり，取り扱いは困難である．

Markov 連鎖 $\{X^x_n\}_{n=0}^\infty$ が重要な基本語彙である理由は，$\cP(E)$ 上の決定論的な力学系 $\{(P^*)^n\delta_x\}_{n=0}^\infty\subset\cP(E)$ を定めるためである．
