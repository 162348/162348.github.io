...
$(E,\cE),(F,\cF)$ を可測空間とする．確率核とは関数 $E\to\cP(F)$ のことをいう（ほとんどの場合可測になる [-@sec-kernel-as-function]）．ただし，$\cP(F)$ は $F$ 上の確率測度全体の集合とした．多くの場合，$\cP(E)$ は Banach 空間内の $w^*$-閉な凸集合になっている．

$E$ 上の確率核 $P:E\to\cP(E)$ が定まると，任意のスタート地点 $x\in E$ に対して，これを起点とする Markov 連鎖 $\{X^x_n\}_{n=0}^\infty\subset\L(\Om)$ が構成できる．

ここで，確率変数 $X^x_n$ を定義するために謎の標本空間 $(\Om,\F,\P)$ を出さざるを得なかった．これは構成のために必要であるが，ご存知の通り $\Om$ の性質が問題になる場面はほとんどない．数学的な本体は核 $P$ なのである．まさに「核」．
...
ここで，$X^x_n$ 自体は確率変数＝可測変数であり，測度論的な存在であるが，$(P^*)^n\delta_x$ は $\cP(E)$ 上の点であり，全体として $\cP(E)$ 上の（確定的な）力学系を定めている．ランダムネスが消えているのである！ただしその代わり，例え $E=\R^d$ であっても，$\cP(E)$ は無限次元空間になっている．

古典力学などが扱う決定論的な運動は，Hamilton の定式化などを通じて，有限次元空間上の幾何学に帰着する．

確率的な系は，確率核 $P$ を通じて，無限次元空間 $\cP(E)$ 上の幾何学に帰着するのである．時代は情報幾何学である．^[ちょうど，非線型力学系において，Koopman　作用素を導入することで，扱う空間が無限次元になってしまう代わりに，ダイナミクスの線型性を回復することに似ている．非線型力学系を扱うにあたって，非線型性の除去が肝心であるように，確率的な系の分析において，ランダムネスを取り除くことが重要であったようだ．]
...
実際，モンテカルロ法が定める $\cP(E)$ 上の力学系を，勾配流として理解することが進んでいる [@Chopin+2023]．
...
この独立性の代数・図式的な特徴付けは，[確率的グラフィカルモデル](../Computation/PGM1.qmd) の分野の結果（[@Hammersley-Clifford1971] の定理など）と重なる部分も多い．

だが，確率核の概念は，純粋に数学的な文献を除いてまだあまりポピュラーな概念であるとは言えないのが現状であろう．

### カーネル法でいう「カーネル」も確率核

実際，カーネル法でいう「カーネル関数」も，（いつでも [RBF カーネル](../Kernels/Kernel.qmd#sec-RBF-kernel) のように確率核になるとは限らないが）核の一種である．
...
カーネル法とは，核 $K:E\to\cS(F)$ に沿って $E$ 上の点を $\cS(F)$ 上に埋め込む方法である．

さらに，再生核 Hilbert 空間の理論により，$\cS(F)$ の部分空間として，内積が $K$ の計算だけで効率的に計算できるようなものが選び出せる．

こうして，$E$ 上のデータ解析を $\cS(F)$ 上に写す非線型対応を導く，数学的に統一された方法が[カーネル法](../Kernels/Kernel.qmd)である．
...
### 核の作用 {#sec-kernel-action}

::: {.callout-tip icon="false" appearance="simple" title="核の作用^[作用は [@Hairer2021-Convergence], [@Kallenberg2017 p.16], [@Dellacherie-Meyer1988 pp.2-3] が同様に定めている．最後の文献によると，この記法は Hunt によるものだという．$T\otimes S$ の存在は [@Gikhman-Skorokhod2004I p.76] 定理II.4.1 で示されており，$\otimes$ を **直積**，$\cdot$ を **畳み込み** と呼んでいる．]"}
...
::: {.callout-tip icon="false" title="命題（核の演算規則）^[(1)は [@Revuz1984 p.9] と [@Dellacherie-Meyer1988 5 p.3]，(2)は [@Dellacherie-Meyer1988 7 p.4]．]"}
