---
title: "Measurability of the Minkowski Sum of Two Sets | Minkowski和の可測性"
...
abstract: For two Borel sets $A,B\in\mathcal{B}(\mathbb{R}^n)$, we cannot expect $A+B$ to be always Borel. We give sufficient conditions for the Minkowski sum $A+B$ to be Borel, and also give a counterexample for the case $n\ge3$.
crossref:
    sec-prefix: 節
    eq-prefix: 式
---
...
A statistician or probability theorist may come across this problem when considering [isoperimetric inequalities](https://en.wikipedia.org/wiki/Isoperimetric_inequality). For example, the one by [@Levy51] and [@Schmidt48] goes as follows:

::: {.callout-tip icon="false"}
## Theorem^[[@Boucheron+2013 p.4], [@Gine-Nickl2021 p.27] Theorem 2.2.1.] [@Levy51]-[@Schmidt48]
Let $S^n\subset\R^{n+1}$ be $n$-dimensional unit sphere and $p\in S^n$ be north pole, $\emptyset\ne A\subset S^n$ be a Borel set, $\mu\in P(S^n)$ be the uniform distribution on $S^n$, and $$C=C(p,\rho)=\Brace{y\in S^n\mid d_{S^n}(x,y)\le\rho}\quad(\rho>0)$$ be a sphererical cap satisfying $\mu(C)=\mu(A)$. Then, the following inequality holds:
$$
\mu(C_\ep)\le\mu(A_\ep),\qquad\ep>0,
$$
where $A_\ep:=\Brace{x\in\R^n\mid d_{S^n}(x,A)\le\ep}$ is closed $\ep$-neighbourhood and $d_{S^n}$ stands for geodesic distance on $S^n$.
:::

Here, 
$$
A_\ep=\Paren{A+B(0,\ep)}\cap\, S^n
$$
so the Borel measurability of $A+B(0,\ep)$ matters.

Of course, $A_\ep$ is $\mu$-measurable. Therefore, we can understand as the above theorem implicitly assumed the Borel probability measure $\mu$ to be completed in the Lebesgue sense.
