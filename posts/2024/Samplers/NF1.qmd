...
さて，輸送問題 [-@sec-transport-problem] は，確率過程 $(X_t)_{t\in[0,1]}$ を，２つの時点 $X_0,X_1$ に関する観測から（帰納バイアス付きで）推定する問題とも等価である．

すると拡散模型は，これを Langevin 拡散により内挿する問題，そして FM をはじめとして Rectified Flow や $\al$-ブレンディングは，これを線型などの単純なダイナミクスにより内挿する問題として理解できる．

一般に，確率過程 $(X_t)_{t\in[0,T]}$ の独立なコピー $X^1,X^2,\cdots,X^n$ から，ランダムな時点 $t\in[0,T]$ での観測
$$
X^1_{t_1},X^2_{t_2},\cdots,X^n_{t_n}
$$
...
はじめ TrajectoryNet [@Tong+2020] という CNF 手法は軌道推定に用いられており，のちに OT-CFM として生成モデリングにも使えることが自覚されたのである．
