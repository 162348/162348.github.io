...
ConvNet [@Xie+2016] 以降，$H_\theta$ のモデリングには深層ニューラルネットワークが用いられるようになっていった．
...
更に，完全に同じ MCMC を走らせ続けるというところから，リサンプリングを取り入れて $\theta$ のアップデートに収束性を保ちながら対応することで，GAN に匹敵する性能と，分布の峰を正確に再現できるという GAN にはない美点を獲得できるという [@Du-Mordatch2019]．
...
尤度 $p_\theta$ の評価を迂回するため，[GAN](../Kernels/Deep3.qmd) にヒントを得た，２つのネットワークを対置させて行う敵対的な学習も考えられている [@Kim-Bengio2016]．
