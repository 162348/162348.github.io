...
[VAE](../Kernels/Deep4.qmd) や [正則化流](NF.qmd) などの生成モデルや，[トランスフォーマー](../Kernels/Deep2.qmd) などの自己回帰モデルも EBM とみなせる．
...
２次元の密度推定に対して，FCE (Flow Contrastive Estimation) により学習させた EBM は，[正則化流](NF.qmd) よりも遥かに小さいネットワークサイズで高い性能を示した [@Gao+2020]．
...
### 正則化スコアマッチング (RSM) [@Kingma-LeCun2010]
...
この $\ep$ だけ摂動されたデータの分布 $\wt{p}$ に対して，スコアマッチング目的関数は
\begin{align*}
...
### Denoising スコアマッチング (DSM) [@Vincent2011]

RSM の場合と違い，次のような展開もできる：
$$
D_F(\wt{p},p_\theta)=\frac{1}{2}\E\Square{\Norm{s_\theta(\wt{X})-\frac{X-\wt{X}}{\sigma^2}}^2_2}.
$$
...
\end{align*}
を最小化する．ただし，$V$ はランダムな $\R^d$ 上のベクトルである．[@Song+2019] では $V\sim\rN_d(0,I_d)$ とすることで Monte Carlo 近似による追加の誤差を回避している．
...
なお，$V\sim\rN_d(0,I_d)$ と取った際の目的関数 $D_{SF}$ は，元の Fisher 乖離度による目的関数に対して，Hutchinson の跡推定量（[正規化流でも出てきた](NF.qmd#sec-Hutchinson)）により Jacobian $Ds_\theta$ を推定したものとも同一視できる．
...
### FCE (Flow Contrastive Estimation) [@Gao+2020]
