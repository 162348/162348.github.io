...
２次元の密度推定に対して，FCE (Flow Contrastive Estimation, 第 [-@sec-FCE] 節) により学習させた EBM は，[正則化流](NF.qmd) よりも遥かに小さいネットワークサイズで高い性能を示した [@Gao+2020]．
...
ノイズ対照推定のノイズ分布を，[正則化流](NF.qmd) を用いて敵対的に高難易度にしていく．この状況で，EBM とフローベースモデルを同時に学習することを考える手法である．
