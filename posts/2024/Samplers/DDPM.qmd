...
[@Ho+2020] ではトランスフォーマー [@Vaswani+2017] 同様の sinusoidal positional encoding を用いてタイムステップ $t$ の情報をデータに統合したものが，パラメータの共有された CNN に与えられる．
...
```{python}
model.eval()
generated_images = torch.load("Files/generated_images0.pt")
```
