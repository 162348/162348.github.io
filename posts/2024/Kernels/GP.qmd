...
平均は $0$ とし，共分散関数は **関連度自動決定** (ARD: Autonatic Relevance Determination) [@MacKay1994], [@Neal1996 p.16] を用いる．
...
# 誘導点 u の生成: ここでは、入力空間上に等間隔に配置された点を使用
u = np.meshgrid(np.linspace(-2, 2, 5), np.linspace(-2, 2, 5))
u = np.hstack((u[0].reshape(-1, 1), u[1].reshape(-1, 1)))  # 形状を (数, 2) に変更

# カーネルの定義
kernel = GPy.kern.RBF(input_dim=2, ARD=True)

# SparseGPClassification モデルの構築
model = GPy.models.SparseGPClassification(X=x, Y=y.reshape(-1, 1), kernel=kernel, Z=u)
