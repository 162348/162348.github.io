...
初めは自然言語処理（特に機械翻訳）の文脈で導入されたが，注意機構は少し修正することで言語，画像，動画などあらゆる系列データのモデリング全体で抜群の性能を発揮する上に，これらを組み合わせることもできる（第 [-@sec-multimodal-transformer] 節）．
...
以上がトランスフォーマーの 95% をなす要素技術である．

次の３要素を含め，典型的には 20 から 24 層を成した深層ニューラルネットワークがトランスフォーマーの全てである．^[層の数などの表現は [@Hashimoto2024] から．]
...
この微調整は **事後調整** (fine-tune) の一種であり，転移学習の一種ともみなせる．^[後述のアラインメントも事後調整の一つであるが，これと区別して，**教師ありの事後調整** (supervised fine-tuning) とも呼ばれる．] この意味でも，さらに使い方の意味でも，種々の応用や下流タスク (downstream task) の基礎となるモデルであることと，そのものでは未完成であることとを強調して，**基盤モデル** (foundation model) とも呼ばれる [@Bommasani+2021]．
...
特に公開事後調整データセットの整備が進んでおり，[Alpaca](https://github.com/tatsu-lab/stanford_alpaca) では Self-Instruct [@Wang+2023] による効率的な alignment 技術が採用されている．
