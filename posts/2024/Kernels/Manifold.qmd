...
[主成分分析と因子分析はいずれも１層の線型 Gauss な潜在変数模型と理解できる](HierarchicalModel.qmd#sec-PCA)．

概して，主成分分析が「低階数近似」ならば，因子分析は「高階数近似」と理解できる [@足立浩平2023]．^[最も，心理学などで用いられる際は，FA の始まり [@Spearman1904] がテスト理論で行ったようにデータを「共通因子部＋独立因子部＋誤差」で説明するという志向があり，必然的に高階近似になる [@足立浩平2023]．]

実際，PCA は [線型な次元縮約法](HierarchicalModel.qmd#sec-dimension-reduction) として古くから研究されている統計手法である．他にも PCA は（非可逆）データ圧縮，特徴抽出，データ可視化に用いられる．^[例えば PCA の結果は，行列の行と列を１つの２次元空間上に書き込む biplot [@Gabriel1971], [@Gower2004] に用いられることも多い．]
...
MDS では類似性行列 $Y$ が与えられるところが，MDU では行と列に互いに異なる対象が並び，その間の親近性が与えられる．

すなわち，各行と各列を与える対象を同一視し，同じ座標に埋め込む場合の MDU が，MDS であったと言える．

### Isomap [@Tenenbaum+2000] {#sec-Isomap}
...
### スペクトル埋め込み [@Mikhali-Partha2001] {#sec-LE}
...
SNE では，$K$-最近傍グラフを用いた [Isomap](#sec-Isomap) を，ハードな帰属からソフトな帰属へ，確率分布を用いて軟化する．
...
UMAP (Uniform Manifold Approximation and Projection) は t-SNE より高速で，大域的構造をより尊重する手法として提案された．
