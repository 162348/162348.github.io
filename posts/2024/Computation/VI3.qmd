...
crossref:
    ...
    fig-title: 図
abstract-title: 概要
abstract: グラフィカルモデルの汎用推論手法である変分 Bayes アルゴリズムを解説する．変分 Bayes 推論とは，事後分布を指定した分布族の中で，KL-距離が最も小さくなるように近似する手法をいう．この分布族に，種々のパラメトリック分布を仮定したり，平均場近似を採用したりすることで，種々の変分 Bayes アルゴリズムが得られる．
---
...
$\al\to1$ の極限では $\KL(p,q)$ に収束し，$\al\to-1$ の極限では $\KL(q,p)$ に収束する．いわば，この２つの量を補間する量である．

$\al=0$ の場合を，Hellinger 距離（の自乗）という．

$\al\le-1$ の場合は $D_\al$ は $\frac{q}{p}$ を含むため，$p$ が零ならば $q$ も零になるようになる．実際，変分近似は分散を過小評価しがちである @sec-VB-problem．
