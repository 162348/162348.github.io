...
グループ内の相関構造については関与せず，デフォルトでは対角行列が指定される．
...
`brm()`関数には，[`autocor`引数](http://paul-buerkner.github.io/brms/reference/autocor-terms.html) が用意されている．経時的繰り返し観測データに対して，同一ユニット間の経時的な相関をモデル化するために用いることができる．
