...
ランダム効果では，ユニットレベルの説明変数 $x_i$ と変動切片項 $\al_{s[i]}$ が相関を持たないという仮定が Gauss-Markov の定理の仮定に等価になるため，これが違反されると推定量の不偏性・一致性が約束されず，推定量の分散も大きくなる．^[[@Hansen2022 p.333] 第12.3節，[@Bafumi-Gelman2007 p.3], [@Hansen2022 p.604]．]
...
以上，解説してきた「ランダム効果モデル」であるが，混合効果モデルとも呼ばれる．
...
これが固定効果モデル（第 [-@sec-fixed-effects-model] 節）の名前の由来である．^[[@Hansen2022 p.625] 17.25節．] 固定効果モデルは，たしかに（ユニットレベルでの回帰係数という意味での）「固定効果」を表す変数しか含んでいない（少なくとも見た目上は）．
...
この $\V[\eta_s]$ は何の仮定も置かれておらず，グループ間の相関構造のモデリングを一手に引き受けている．EM アルゴリズムが提案されたばかりの頃 [@Laird-Ware1982] では，共分散構造にパラメトリックな仮定をおいていたが，現代ではこれを取り去った最尤推定法・ベイズ推定法（２つは計算法が違うだけで本質的に同じ）が主流である．

共分散構造が $0$ になるのが好ましくない場合は，Wishart 事前分布を仮定することでこれが回避される（最尤法の文脈では，penalized likelihood と等価になる）ことが，ベイズ MAP 推定の文脈で [@Chung+2015]，最尤推定の文脈で [@Chung+2013] によって示されている．

また，計算上の都合で特定の共分散構造を仮定することもある [@Kincaid2005]．
