...
すなわち，第 [-@sec-example] 節のモデルの比較 [-@sec-model-comparison] で扱った，**観測レベルランダム効果** (OLRE: Observation-level Random Effects) の方法は，[観測毎に $\eta_{it}$ というランダム切片項を追加するだけで，本質的には $y$ に対する Poisson-対数正規混合分布族によるモデリングを実施する]{.underline} ということに対応するのである．^[[Solomon Kurtz (2021)](https://solomonkurz.netlify.app/blog/2021-07-12-got-overdispersion-try-observation-level-random-effects-with-the-poisson-lognormal-mixture/#negative-binomial-counts) による解説，[RPubs](https://rpubs.com/INBOstats/OLRE) や [Section3.1 @Hamura+2025] も参照．]

全く同様に，$\eta_{it}$ に Gamma 分布を仮定すれば，$y_{it}$ を負の二項分布でモデリングすることと等価になる．

過分散に対応するモデルとして，Poisson 分布の他に，Poisson 分布と対数正規分布の混合分布であるとか，負の二項分布であるとかを覚えるよりも，このように構造的に導き出す手法の方が探索的な用途には向いているとも言えるだろう．
