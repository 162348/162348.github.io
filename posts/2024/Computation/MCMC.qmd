...
![ポテンシャル $U$ のプロット](Files/potential.svg){width=50%}
...
統計力学によれば，$\beta=1$ で定まる温度とポテンシャル $U$ を持つ正準集団は $e^{-U}$ の分布に従います．したがってその物理過程をシミュレートすることで，この分布からサンプリングすることが可能になるでしょう．
...
samples_MD = md(num_samples * 10, 0.0, 1.0, timestep=0.01)
...
ここでもう一度 Metropolis 法の軌跡 [-@fig-MHtraj] を見てみましょう．
