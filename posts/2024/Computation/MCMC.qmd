...
以下は，5月24日 10:30~12:30 （コアタイム：10:30~11:10）に行われた [2024年度統数研オープンハウス](../../../static/Sessions.qmd#sec-ISM-openhouse2024) ポスターセッション（掲載 No. E1）に於て発表された上付のポスターに関する解説記事です．
:::
...
このポテンシャルに関する Boltzmann-Gibbs 分布 $\pi\propt e^{-\beta U}$ は次のような形になります：
...
低エネルギー状態が現れやすく，エネルギーが上がるにつれて急激に現れにくくなることがわかります．以降，$\beta=1$ としましょう．

### 分子動力学法

統計力学によれば，$\beta=1$ で定まる温度とポテンシャル $U$ を持つ Boltzmann-Gibbs 分布 $e^{-U}$ は，温度 $T=\frac{1}{k_B\beta}$ を持つ熱浴に接している力学系を，長時間シミュレーションして時間平均を取ることでサンプリングできるはずです（エルゴード仮説）．

このように，力学に基づいて物理過程を数値シミュレーションをすることでサンプリングをするという技法を [**分子動力学法**](https://ja.wikipedia.org/wiki/%E5%88%86%E5%AD%90%E5%8B%95%E5%8A%9B%E5%AD%A6%E6%B3%95) ともいいます．

これを実際にやってみます．ポテンシャル（ @fig-2 ）内を運動する粒子，またはポテンシャル（ @fig-2 ）を持つ（非調和）振動子などの力学系を考えます．これを温度 $T=\frac{1}{k_B\beta}$ を持つ熱浴と相互作用させます．例えば，ポテンシャル（ @fig-2 ）の $x=0$ の位置に半透性の壁を置き，確率 $1/2$ でこの温度 $T$ の壁の粒子と弾性衝突するとします．

壁の速度は Maxwell の境界条件から与えられるものとすれば，次のようにして粒子の位置 $x$ がシミュレートできます：
