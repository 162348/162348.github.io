...
[医療経済学](https://ja.wikipedia.org/wiki/医療経済学) などにおいて，[**医療技術評価**](https://ja.wikipedia.org/wiki/医療技術評価) (HTA: Health Technology Assessment) では，新たな医療技術を臨床試験で評価し，リスクやコストを勘案して新技術の既存法との効果や安全性を評価・比較する決定理論的な枠組みである．
...
### Polyhazard モデルのベイズ推論

第 [-@sec-polyhazard-model] 節で述べたモデルの推定は HTA で多く試みられている．
...
[@Hardcastle+2024] では完全なベイズ階層モデルの推定を試みている．ここではそのモデルの詳細を紹介する．

$\theta_k=(\nu_k,\mu_k)\;(k\in[K])$ に関しては，
$$
\log(\nu_k)=\al_k\sim\rN(0,\sigma_\al^2)
$$
$$
\log(\mu_k)=\beta_{k,0}+\sum_{j\in\Brace{j\in[p]\mid\gamma_{kj}=1}}x_j\beta_{k,j}\tag{2}
$$
$$
\beta_{k,0}\sim\rN(0,\sigma_{\beta_0}^2)
$$
とし，$\sigma_\alpha^2=2,\sigma_{\beta_0}^2=5$ は固定してしまう．加えて，
$$
\beta_{k,j}\sim(1-\om)\delta_0+\om\operatorname{N}(0,\sigma_\beta^2)
$$
と spike-and-slab 事前分布 [@Mitchell-Beauchamp1988] を仮定し，変数選択を促進する．

ここで $\phi=(\om,\sigma_\beta)$ がハイパーパラメータとして残っている．これには
$$
\om\sim\Beta(a,b)
$$
$$
...
K\sim\Pois_{>0}(\xi)
$$
個選ぶこととする．ただし，[@Hardcastle+2024] では $\xi=2$ とする．この根拠は
$$
\P[K>4]\approx0.061
$$
であることとしている．
...
[@Hardcastle+2024] では完全なベイズ polyhazard モデルに対するサンプラーを構成している．ここではこれを紹介する．

### ２つの問題

#### Trans-dimensionality
...
加えて $h_k$ の関数形も途中で変わる（第 [-@sec-prior-of-PX] 節）．
...
$(K,D,\gamma,\phi)$ を固定した下での $\theta\in\R^{2K+\abs{\gamma}_1}$ のサンプリングには，多峰性分布につよい Zig-Zag サンプラーを用いる．

#### Zig-Zag サンプラーについて {.unnumbered .unlisted}
