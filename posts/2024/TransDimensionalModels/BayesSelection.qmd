...
このアプローチは [@George-McCulloch1997] らによって創始された．この方法では
$$
...
$$
\Sigma(\sigma,\gamma):=\sigma^2(X^\top_\gamma X_\gamma)^{-1}.
$$
...
[@Yang+2016] は MH 法の計算複雑性を解析し，$p$ が大きい場合にも計算量が線型にしか増加しない MH 法を提案した．
...
ベイズ変数選択とは $\Gamma=\{0,1\}^p$ 上の事後分布を計算することであるが，これを効率的に行う MCMC を構成することが中心的な問題になる．
...
対称かつ局所的なサンプラーの中では，採択率が高いほど効率が良い [@Peskun1973], [@Tierney1998]．そこで離散空間上の MH 法の採択率を上げるために，現在位置の周囲の点の情報を収集して次の動きを決めるサンプラーが **Informed MCMC** の名前の下で開発されている．

この中でも LIT (Locally Informed and Thresholded proposal) [@Zhou+2022] は次元 $p$ に依存しない収束速度を達成する．
