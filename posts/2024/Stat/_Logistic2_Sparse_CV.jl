...
for i in 1:length(α_list)
    result = optimize(ξ -> U(ξ, x_Sparse[i], y_Sparse[i]), [0.0], LBFGS())
    push!(ξ_stars, Optim.minimizer(result)[1])
end

a_Affine1(ξ, θ, x, y) = pos(θ * ∇U(ξ_stars[1],x,y)) + C(ξ, θ, x, y) * abs(ξ - ξ_stars[1])
ab_Affine1(ξ, θ, x, y, ::ZigZag1d) = (a_Affine1(ξ, θ, x, y), b_Affine(ξ, θ, x, y))
a_Affine2(ξ, θ, x, y) = pos(θ * ∇U(ξ_stars[2],x,y)) + C(ξ, θ, x, y) * abs(ξ - ξ_stars[2])
ab_Affine2(ξ, θ, x, y, ::ZigZag1d) = (a_Affine2(ξ, θ, x, y), b_Affine(ξ, θ, x, y))
a_Affine3(ξ, θ, x, y) = pos(θ * ∇U(ξ_stars[3],x,y)) + C(ξ, θ, x, y) * abs(ξ - ξ_stars[3])
ab_Affine3(ξ, θ, x, y, ::ZigZag1d) = (a_Affine3(ξ, θ, x, y), b_Affine(ξ, θ, x, y))
...
function λj(j::Int64, ξ, θ, ∇U, x, y; ξ_star=1.0)
    Eʲ = ∇U(ξ_star, x, y) + ∇U(1, j, ξ, x, y) - ∇U(1, j, ξ_star, x, y)
    return pos(θ * Eʲ)
end

function ZZ1d_CV(∇U, ξ, θ, T::Float64, x::Matrix{Float64}, y::Vector{Float64}, Flow::ZigZagBoomerang.ContinuousDynamics; rng=Random.GLOBAL_RNG, ab=ab_Affine, ξ_star=1.0)
    t = zero(T)
    Ξ = [(t, ξ, θ)]
    num = acc = 0
    epoch_list = [num]
    ...
    while t < T
        ...
        t, ξ, θ = move_forward(τ, t, ξ, θ, Flow)
        j = rand(1:length(y))
        l, lb = λj(j, ξ, θ, ∇U, x, y; ξ_star=ξ_star), λ_bar(τ, a, b)  # λ が真のレート, λ_bar が affine proxy
        num += 1
        if rand()*lb < l
            acc += 1
            if l > lb + 0.01
...
function getESSperEpoch_SS(ab, ZZ, T ,dt, x, y; ξ0=0.0, θ0=1.0, ξ_star=1.0)
    trace, epochs, acc = ZZ(∇U, ξ0, θ0, T, x, y, ZigZag1d(); ab=ab, ξ_star=ξ_star)
...
function experiment_Sparse(N, T, dt; ξ0=0.0, θ0=1.0, α_list=[1, 0.1, 0.01, 0.001])
    ...
    for _ in 1:N
        ...
        # ESSs_SS = []
        # ESSs_IS = []
        push!(ESSs_CV, getESSperEpoch_SS(ab_Affine1, ZZ1d_CV, T, dt, x_Sparse[1], y_Sparse[1]; ξ0=ξ0, θ0=θ0, ξ_star=ξ_stars[1]))
        push!(ESSs_CV, getESSperEpoch_SS(ab_Affine2, ZZ1d_CV, T, dt, x_Sparse[2], y_Sparse[2]; ξ0=ξ0, θ0=θ0, ξ_star=ξ_stars[2]))
        push!(ESSs_CV, getESSperEpoch_SS(ab_Affine3, ZZ1d_CV, T, dt, x_Sparse[3], y_Sparse[3]; ξ0=ξ0, θ0=θ0, ξ_star=ξ_stars[3]))
    ...
    return mean(ESSs_sum_CV, dims=2), var(ESSs_sum_CV, dims=2)
end
...
ESS_CV, var_ESS_CV = experiment_Sparse(N, T, dt; ξ0=0.0, θ0=1.0, α_list=α_list)

using JLD2

@save "Logistic2_Experiment4_Sparse.jld2" ESS_CV var_ESS_CV
