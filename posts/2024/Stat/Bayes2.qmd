...
::: {.callout-tip appearance="simple" icon="false" title="planted ensemble^[こうして生成されたスピングラス系を，planted ensemble と呼び，このシナリオを機械学習の訓練過程に準えて teacher-student scenario とも呼ぶのが [@Zdeborova-Krzakala2016 p.462] の用語である．これを [@Iba1999 p.3881] は符号理論に準えて，encoding-decoding scenario と呼ぶべき設定で解説している．]"}
...
後者に関する平均は $\brac{X}:=\E[X|Y]$ で表す一方で，前者に関する平均はよく $[-]$ で表される．

従来のスピングラス理論では，disorder $Y_{ij}$ に関する平均 $[-]$ を先に取ることで，従来の Ising 模型と同様の議論に持ち込む戦略が取られる．
...
$$
f\approx-\lim_{N\to\infty}\frac{1}{N\beta}\lim_{n\to\infty}\frac{[Z^n]-1}{n}.
$$

一方で，次の値も良い近似として用いられることがある：
$$
...
この要約伝搬法は Bethe 近似に関係がある [@Kabashima2005]．

:::

一方で，planted ensemble に注目することで，従来考えられなかった解析が進むことがある．^[[@Zdeborova-Krzakala2016 p.483] 2.5.3 節 _Quiet Planing_ を参照ください．]
...
[@Nishimori1980] では，この Hamiltonian ([-@eq-Hamiltonian]) が，真値 $x^*\in\{\pm1\}^N$ が定める次の変換 $\wt{-}$ に対して不変であることが利用された：
$$
\wt{x_i}:=x_ix_i^*,
$$
$$
...
この $\{y_{ij}\}$ に関する Edwards-Anderson モデル $H$ において，$\rho$ と $\beta$ の値の関係が系の振る舞いを決定し，その相図は元々の planted ensemble の性質と同一視できる．

#### 西森ライン上での西森条件

このとき，Edwards-Anderson モデルは $\beta=\beta^*$ （式 [-@eq-Nishimori-temperature]）の場合に限り，内部エネルギーや比熱の上限が厳密解を持つ．^[[@Nishimori1980] の報告である．[@西森秀稔1999 pp.53-]，[@Iba1999 p.3879] 式 (25) も参照．] 例えば，平均次数 $c$ を持つランダムグラフ上の Edwards-Anderson モデルの西森温度における内部エネルギーは
$$
...
一般のスピングラス模型に対してこのような変換が見つかるわけでもなければ，その変換の物理的な意味が定かでない．

しかし，西森ラインとは，一般の planted spin glass model （第 [-@sec-teacher-student-senario] 節）において，モデルが正しく特定されていること $\beta=\beta^*$ の特別な場合と理解できる [@Iba1999]．
