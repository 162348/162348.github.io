...
#| output: false
using JLD2
@load "Logistic2_Experiment4_Sparse.jld2" ESS_SS var_ESS_SS ESS_IS var_ESS_IS
```
...
#| code-summary: プロット
using LaTeXStrings
p = startPlot(α_list, ESS_SS, sqrt.(var_ESS_SS); label="ZZ-SS", xlabel=L"Sparsity $\alpha$")
p = addPlot(p, α_list, ESS_IS, sqrt.(var_ESS_IS); label="ZZ-IS")
display(p)
```
