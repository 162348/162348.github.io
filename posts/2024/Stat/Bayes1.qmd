...
csl: ../../../apa.csl
abstract-title: 概要
abstract: ノイズ付きで観測された情報を復元するデノイジング問題は，ベイズ推定問題として扱える．これを統計力学の観点からランダムエネルギーモデルとして解析することで，データ数無限大の極限における振る舞いを理解できる．一般に，ベイズ統計モデルはスピングラスモデルと同一視することができる．
format:
    html:
        code-fold: false
execute:
...
こうして，誤り訂正符号の文脈では，Bayes 事前確率・事後確率が純粋に確率（または頻度）としての解釈を持つ．この２つの formalism の類似性は [@Iba1999 p.3876] でも指摘されている．
...
::: {.callout-tip title="命題 [@西森1980]^[[@Krzakala-Zdeborova2024 p.123] 定理13，[@Iba1999 pp.3876-3877]．]" icon="false"}
...
::: {.callout-tip title="命題^[[@Krzakala-Zdeborova2024 p.125]定理15．証明は 7.B 節 p.133 を参照した．]" icon="false"}
