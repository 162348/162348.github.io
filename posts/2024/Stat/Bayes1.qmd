...
csl: ../../../apa.csl
abstract-title: 概要
abstract: ノイズ付きで観測された情報を復元するデノイジング問題は，ベイズ推定問題として扱える．これを統計力学の観点から解析することで，データ数無限大の極限における振る舞いを理解できる．一般に，ベイズ統計モデルはスピングラスモデルと同一視することができる．
format:
    html:
        code-fold: false
execute:
...
上の一様分布
$$
p(x^{(i)})=\frac{1}{d}=\frac{1}{2^N}
$$
とする．ただし，$x^{(i)}$ は第 $i\in[d]$ 成分のみが $1$ の one-hot ベクトルとした．

このとき，事後分布 $p(x|y)dx$ もやはり $\Delta_d$ 上の分布で，その質量関数は
\begin{align*}
