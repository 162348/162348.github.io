...
送信符号は１つの値 $x^*\sim p(x)dx$ であったとし，この単一の入力 $x^*$ を $n$ 回独立に観測する：
$$
...
$$
p(y|x)dy=\rN(x,\sigma^2)^{\otimes n}(dy)
$$
...
この場合，分配関数 $Z(\b{y})$ は，情報源 $p(x)$ と Gauss 型通信路 $p(y|x)$ で与えられた周辺モデル $p(y)$ と，完全にランダムとした場合 $\rN(0,\sigma^2)^{\otimes n}$ との尤度比，または [**ベイズ因子**](https://ja.wikipedia.org/wiki/ベイズ因子) になっている．
...
$$
上の一様分布とする．このとき，事後分布 $p(x|y)dx$ もやはり $\Delta_d$ 上の分布で，その質量関数は
$$
p(x^{(i)}|y)=\frac{1}{\cZ}\frac{1}{2^N}\exp\paren{\frac{2y_i-1}{2\sigma^2/N}},
$$
$$
\cZ:=\frac{1}{2^N}\sum_{i=1}^d\exp\paren{\frac{Ny_i}{\sigma^2}-\frac{N}{2\sigma^2}},
$$
と表される．ただし，$x^{(i)}$ は第 $i\in[d]$ 成分のみが $1$ の one-hot ベクトルとした．
...
$$
\pp{\Phi(\beta)}{\beta}=\frac{\E[\brac{X}^2]}{2}
$$
が成り立つ．よって，$\Phi$ は $\beta$ の一次関数の形であるはずである．

実際，次が示せる：

::: {.callout-tip title="命題^[[@Krzakala-Zdeborova2024 p.125]定理15．]" icon="false"}
$\Delta:=\sigma^2$ の関数として，自由エネルギー密度 $\Phi$ は次で定まる関数 $f:\R\to\R$ に一致する：
$$
f(\Delta):=
\begin{cases}
...
::: {.callout-tip title="補題^[[@Krzakala-Zdeborova2024 p.125]補題８．]" icon="false"}
$$
...
::: {.callout-tip title="補題^[[@Krzakala-Zdeborova2024 p.126]補題９．]" icon="false"}
$$
...
$N\to\infty$ の極限において，$\Delta>\Delta_c$ の場合，最小自乗誤差（MMSE）は $1$ であり，何をどうしても復号することはできない．Gauss ノイズ $\Delta=\sigma^2$ が大きすぎるのである．

一方で，$\Delta<\Delta_c$ の場合，$\partial_\beta f=\frac{1}{2}$ であり，従って MMSE は $0$ になる．よって完全な誤りのない復号が可能であるはずである．

::: {.callout-important appearance="simple" icon="false"}
設定（第 [-@sec-settings] 節）が与えられたならば，立ち所に，$\Delta$ が大きいほど復号が難しいことは予想がつく．しかし，ある閾値 $\Delta_c$ に依存して，少なくとも $N\to\infty$ の極限では，

* 全く異なる２つの振る舞いをすること
* ２つの振る舞いのみに分類される（ある一定以上の確率で復号が可能である，などの中間状態がない）

ことは驚きである．
:::

実は，$\Delta>\Delta_c$ の際，自由エネルギー $F_N$ 自体も $0$ に（指数）収束している．$F_N$ とは，ランダムエネルギーモデル
$$
q(y)dy:=\rN_d\Paren{y,\frac{\Delta}{N}I_d}
$$
と $p(y)$ との距離であったから，そもそも観測が，完全に乱雑な状態と高速に一致していくのである．

### 自由エネルギーのさらに鋭い評価

::: {.callout-tip title="命題（KL 乖離度は指数収束する）^[[@Krzakala-Zdeborova2024 p.132]補題10．]" icon="false"}

$\Delta>\Delta_c$ のとき，任意の $N\in\N^+$ について次が成り立つ：

\begin{align*}
    NF_N(\Delta)&=\KL\Paren{p(y)\,\bigg|\,q(y)}\\
    &\le Ne^{-KN}
\end{align*}
