...
$$
という項を加えて得る Zig-Zag サンプラーともみなすことができる．

こうして定義された $\gamma_i$ が非負関数である限り，平衡分布は $\wt{\pi}$ のままであるが，非負性は関数 $(-)_+$ の凸性から従う．
...
しかし
$$
\max_{k\in[n]}\Paren{\theta\partial_iU^k(x+\theta t)}_+\le M_i
$$
...
ZZ-CV が平衡に至っている場合は $x$ はほとんど $x^*$ に集積するため，
$$
...
$$
X^j\iidsim\rN(\xi_0,\sigma^2),\qquad j\in[n],
$$
分散 $\sigma^2$ は既知として，位置母数 $\xi\in\R$ を推定する問題を考える．

事前分布を $\rN(0,\rho^2)$ とすると，定数の違いを除いて
\begin{align*}
    U(\xi)&=\frac{\xi^2}{2\rho^2}+\frac{1}{2\sigma^2}\sum_{j=1}^n(\xi-x^j)^2\\
    &=\frac{1}{n}\sum_{j=1}^n\paren{\frac{\xi^2}{2\rho^2}+\frac{n}{2\sigma^2}(\xi-x^j)^2}=:\frac{1}{n}\sum_{j=1}^nU^j(\xi)
\end{align*}
であるから，
\begin{align*}
    U'(\xi)&=\frac{\xi}{\rho^2}+\frac{1}{\sigma^2}\sum_{j=1}^n(\xi-x^j)\\
    &=\frac{\xi}{\rho^2}+\frac{n}{\sigma^2}(\xi-\ov{x}),
\end{align*}
$$
U''(\xi)=\frac{1}{\rho^2}+\frac{n}{\sigma^2}.
$$

従って，Zig-Zag 過程のイベントの強度関数は
\begin{align*}
    m(t)&=\Paren{\theta U'(\xi+\theta t)}_+\\
    &=\paren{\frac{\theta(\xi+\theta t)}{\rho^2}+\frac{\theta}{\sigma^2}\sum_{j=1}^n(\xi+\theta t-x^j)}_+\\
    &=\paren{\frac{\theta\xi}{\rho^2}+\frac{\theta}{\sigma^2}\sum_{j=1}^n(\xi-x^j)+t\paren{\frac{1}{\rho^2}+\frac{n}{\sigma^2}}}_+
\end{align*}
と表せ，これは１次関数であるから直接のシミュレーションが可能である．
...
"""
    ...
    x からスタートする速度 θ の１次元粒子が原点 0 に到着する時刻を返す
"""
...
"""
    ∇ϕ: gradient of the negative log-density
    (x,θ): initial state
    T: Time Horizon    
    c: tuning parameter
...
"""
function ZZ(∇ϕ, x, θ, T, c, k, Flow::ZZB.ContinuousDynamics; adapt=false, factor=2.0, rng=Random.GLOBAL_RNG)
    t = zero(T)
    Ξ = [(t, x, θ)]
    num = acc = 0
    a, b = ab(x, θ, c, Flow)
...
続いて，次の等式が成り立つ：
$$
U'(\xi_*)+(U^j)'(\xi)-(U^j)'(\xi_*)=U'(\xi),\qquad\xi,\xi_*\in\R.
$$

このモデルにおける MAP 推定量は
$$
\wh{\xi}:=\frac{\ov{x}}{1+\frac{\sigma^2}{n\rho^2}}
$$
である．

### 非一様な部分サンプリング

当然，必ずしも一様なサブサンプリング $K\sim\rU([n])$ を行う必要はない．剪定の手続きを棄却法とみると，重点サンプリングのアイデアを導入することで制御変数に依らない分散低減が狙えるだろう [@Sen+2020 importance subsampling strategy]．

特に，比例的高次元極限や，不均衡データに対する[ロジスティック回帰](../Stat/Logistic.qmd)では，事後分布が十分な集中性を持たないために制御変数の方法 [-@sec-ZZ-CV] が十分な効率改善を示さない．

一般の $K\sim\mu_i\in\cP([n])$ に対して，
$$
m_{1i}^k(t):=\Paren{\theta_i\partial_iU_1^K(\xi+t\theta)}_+
$$
$$
m_{0i}(t):=\Paren{\theta_i\partial_iU_0(\xi+t\theta)}_+
$$
と定める．
