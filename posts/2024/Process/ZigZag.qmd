...
$X\in\R^d$ が位置を，$\theta\in\{\pm1\}^d$ が速度を表すと解する．すなわち，Zig-Zag 過程は全座標系と $45$ 度をなす方向に，常に一定の単位速さで $\R^d$ 上を運動する粒子と見れる．

換言すれば，決定論的なフロー $\phi_{(x,\theta)}:\R\to\R^d$ を，次の微分方程式系で定める：
$$
\dd{\phi_{(x,\theta)}(t)}{t}=\theta,\quad \dd{\Theta_t}{t}=0.
$$

### アルゴリズム {#sec-algorithm}

$Z$ は次のようにしてシミュレーションされる：
...
T_i\deq M_i^{-1}(E_i)
$$
が成り立つ．^[この逆関数 $M_i^{-1}$ が intractable である場合でも，[thinning と呼ばれる方法](PureJump.qmd#sec-simulation) によって $T_i$ は正確なシミュレーションが可能である．]
...
PDMP の妙は全てレート関数 $\lambda_1,\cdots,\lambda_d$ に宿っている．Zig-Zag 過程の特徴は，レート関数の選択自由度が大きい点にある．
...
上述のリフレッシュレート $\lambda_1,\cdots,\lambda_d$ に対して，定義 [-@sec-algorithm] で定まる Zig-Zag 過程 $Z$ は次の分布 $\mu=\pi\otimes\rU(\{\pm1\}^{d})\in\cP(E)$ を不変にする：
$$
\mu(dxd\theta)=\frac{1}{2^d}\frac{e^{-U(x)}}{\cZ}\,dxd\theta
$$
...
## Super Efficient Bayesian Inference
