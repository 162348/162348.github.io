...
PDMP は拡散項を持たない Lévy 過程とも理解できる．^[ただし，ジャンプが頻繁すぎないことも必要である．[@Davis1993 p.60] 過程24.4 では，有界区間上のジャンプは有限回であると過程している：$\E[N_t]<\infty\;(t\in\R_+)$．すなわち，A 型までの加法過程を PDMP というのであって，B 型と C 型は，シミュレーションが容易な連続時間過程であるという美点を逃してしまう．加法過程の分類については，この [稿](Levy.qmd) も参照．]
...
::: {.callout-tip title="（非形式的な）定義 (PDMP: Piecewise Deterministic Markov Process)^[[@Davis1993 p.58] に倣った．]" icon="false"}

$E$ を有限次元多様体，$\fX$ をその上の滑らかなベクトル場，$\lambda\in\L_\loc(E)_+$ を局所可積分な非負関数，$Q:E\times\B(E)\to[0,1]$ を確率核で，$Q(x,\{x\})=0$ を満たすものとする．

３組 $(\fX,\lambda,Q)$ が定める **PDMP** $X$ とは，あるスタート地点 $x\in E$ から，一様乱数 $U_1\sim\rU([0,1])$ と $\fX$ が定めるフロー $\phi$ が定めるジャンプ時刻
$$
T_1:=\inf\Brace{t\ge0\,\middle|\,\exp\paren{-\int^t_0\lambda(\phi(s,x))ds}\le U_1}
$$
まで，フローの通りに移動するとし：
$$
X_t=\phi(t,x),\quad t< T_1,
$$
その後，新しい位置は確率核 $Q$ に従って決まる：
$$
X_{T_1}\sim Q(\phi(T_1,x),-).
$$

これの手続きを，$X_{T_1}$ を次のスタート地点として繰り返すのが特性量 $(\fX,\lambda,Q)$ が定める PDMP $X$ である．

:::

こうして得た PDMP は，Feller-Dynkin 過程であるとも限らないのに，時間的に一様な強 Markov 過程である．^[[@Davis1993 p.64] 定理25.5も参照．]
...
$X$ を PDMP とする．このとき，$X$ の拡張生成作用素 $\wh{A}$ について，可測関数 $f\in\L(E)$ が $f\in\D(\wh{A})$ であるとは，次と同値：
