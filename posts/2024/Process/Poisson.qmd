...
強度 $\lambda>0$ を持つ Poisson 計数過程とは，$\R_+$ 上のレート $\lambda>0$ を持つ Poisson 点過程（第 [-@sec-Poisson-point-process-on-R] 節）$\eta$ に対して，
$$
N_t(\om):=\eta(\om,[0,t])
$$
で定まる Lévy 過程である．
...
`yuima`パッケージでは，Poisson 計数過程は複合 Poisson 計数過程の特別な場合として扱うため，シミュレーション法は第 [-@sec-simulating-CP] 節で扱う．
...
$\lambda>0$ を正数，$\mu\in\cP(\R^d)$ を確率分布とする．**複合 Poisson 過程** $\CP(\lambda,\mu)$ とは，強度 $\lambda$ を持つ Poisson 過程 $N$ が定める Lévy 過程 $\{X_t\}\subset\L(\Om;\R^d)$ をいう：
$$
