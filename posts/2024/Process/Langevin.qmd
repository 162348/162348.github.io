...
&\le\E_x\SQuare{\sup_{t\in[0,T]}\paren{1-1_{[0,\tau_n]}(t)}\abs{\wh{L}G(X_t)}}\\
&=\E_x\SQuare{(1-1_{[0,\tau_n]}(T))\sup_{t\in[0,T]}\abs{\wh{L}G(X_t)}}\xrightarrow{n\to\infty}0.
\end{align*}
ここで $t\mapsto\wh{L}G(X_t)$ の局所有界性４ $\sup_{t\in[0,T]}\abs{\wh{L}G(X_t)}<\infty$ を用いた．
]
