...
1. $\abs{L_t}$ と OU 過程を比較することで，$\abs{L_t}$ が爆発しないことが判る．
      ...
      続いて，ドリフト係数が局所有界であるため，[@Bhattacharya1978] と同様の議論により $\ell_k$-既約かつ強 Feller である．

      強 Feller かつ $\ell_k$-既約ならば，任意のコンパクト集合は小集合である [@Tweedie1994 定理5.1]．

      非周期性は，任意のスケルトンも $\ell_k$-既約であることから従う．
2. [@Ikeda-Watanabe1981 5.4節] により，$\pi$ は $L$ の不変分布であることが，生成作用素
      ...
      f\in C^2(\R^k)
      $$
      に関して不変であることから従う．$C^2(\R^k)$ は，爆発しない拡散過程について，分布を定める (distribution-determining class)．

      不変分布を持つから，再帰的ではある [@Tweedie1994 定理2.3]．
      
      また連続な過程であるから，これは Harris 再帰性を意味する．[@Meyn-Tweedie1993 定理6.1]．

      不変確率分布を持つ非周期的な Harris 再帰的な Markov 連鎖であるから，全変動収束が従う．
...
::: {.callout-note icon="false" title="証明"}
1. 非空コンパクト集合 $C\compsub\R^k$ は小集合であるから，これは成り立つ．
2. 任意の $\delta>0$ について，$\delta$-スケルトンも幾何エルゴード的である．この $\delta$-スケルトンに関する $C$ への到着時刻は $\tau^\delta_C$ よりも必ず大きくなる．

      $L_t$ は強 Feller であるから，$C$ は $\delta$-スケルトンに関しても小集合である．あとは，Markov 連鎖に関する幾何エルゴード定理より従う．
:::
