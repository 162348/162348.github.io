...
1. $\abs{L_t}$ と OU 過程を比較することで，$\abs{L_t}$ が爆発しないことが判る．続いて，ドリフト係数が局所有界であるため，[@Bhattacharya1978] と同様の議論により $\ell_k$-既約かつ強 Feller である．強 Feller かつ $\ell_k$-既約ならば，任意のコンパクト集合は小集合である [@Tweedie1994 定理5.1]．非周期性は，任意のスケルトンも $\ell_k$-既約であることから従う．
...
f\in C^2(\R^k)
$$
に関して不変であることから従う．$C^2(\R^k)$ は，爆発しない拡散過程について，分布を定める (distribution-determining class)．[@Tweedie1994 定理2.3] より，不変分布を持つから，再帰的ではある．また連続な過程であるから，これは Harris 再帰性を意味する．[@Meyn-Tweedie1993 定理6.1] から，全変動収束が従う．
...
1. $L$ が一様エルゴード的ならば，ある非空コンパクト集合 $C\compsub\R^k$ について，ある $\kappa>1,\delta>0$ が存在して，任意のスタート地点 $x\in \R^k$ について，
$$
