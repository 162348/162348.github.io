...
$J_{x,y}$ の符号がバラバラである場合，これをスピングラスの模型という．^[[@Mezard-Montanari2009 p.168] [@西森秀稔2003 p.16] など．]

このような模型では，低音秩序相が消えて，スピンがバラバラである状態（スピングラス相）も安定たり得ることがわかっている．

特に，安定な状態が複数存在し，温度を少し変えるだけで全く性質の異なる別の状態へ系が移ることもよくある．ポテンシャルが強い多峰性を示すのである．
...
$(J_{x,y})_{\abs{x-y}=1}$ を，グラフのエッジの集合上に定義された独立な確率場とし，外場を考えないものを，**Edwards-Anderson 模型** という：
$$
H(\sigma)=-\sum_{\abs{x-y}=1}J_{x,y}\sigma(x)\sigma(y).
$$

確率変数 $J_{x,y}$ の平均（配位平均）と，アンサンブル平均という２つの平均を扱う必要がある点で極めて難しい模型となっている．
...
EA 模型を，平均場近似によって解けるようにした近似模型である．

[@Sherrington-Kirkpatrick1975] はさらに，これを EA 模型の無限レンジ極限における厳密解として関連づけた．その際にもレプリカ法が重要な手法として用いられた．

SK 模型も，常磁性相と強磁性相に加えて，スピングラス相を示す．

著者のうちの Scott Kirkpatrick は simulated annealing [@Kirkpartick+1983] の開発者でもある．

#### Hopfield 模型 [@Hopfield1982]

のちにスピングラスの理論は **連想記憶** にも応用され，広く情報処理の問題を統計力学の技法によって研究する情報統計力学という新たな分野が開拓された．

[連想記憶のニューラルネットワーク](../Kernels/Deep.qmd#sec-Hopfield) は，素子 $\{S_i\}_{i=1}^N\subset\Map(T;\{\pm1\})$ からなるとき，
$$
...
[@Minlos2000], [@西森秀稔2003], [@Altieri-Baity-Jesi2024], [@Chatterjee2023]