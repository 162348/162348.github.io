...
* 部分集合 $\bZ,\Q\subset\R$ についても同様．特に $\N:=\bZ_+$．^[この運用は [@Jacod-Protter2012] に一致する．記法 $\N$ は [@Villani2009] などでは正整数の全体 $\N=\Brace{1,2,3,\cdots}$ と定められている．]
...
* 関数 $f:\R\supset T\to\X$ が [**第一種不連続**](https://ja.wikipedia.org/wiki/%E4%B8%8D%E9%80%A3%E7%B6%9A%E6%80%A7%E3%81%AE%E5%88%86%E9%A1%9E) であるとは，常に左極限を持つ右連続関数であることをいい，このような関数の全体を $D(T;\X)$ で表す．^[「第一種不連続」とは [@伊藤清1991-確率論 p.227] の用語．[@LeGall2016 p.168] では $\mathbb{D}(\X)$, [@Jacod-Shiryaev2003 p.325] では $\mathbb{D}(\X)$ と表す．]
...
\Delta x(t):=x(t)-x(t-)
$$
で表す．^[[@Jacod-Shiryaev2003 p.325] に倣った．]
