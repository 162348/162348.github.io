...
* 期待値作用素と確率測度の引数は
$$\E[X],\quad\P[X\in A]$$
と角括弧内に記する．^[[@吉田朋広2006-数理統計 p.5] に倣った．筆者は $\E,\P$ のいずれも作用素と見る立場に立つためである．[@Gine-Nickl2021] は $E[X],\mathrm{Pr}\{X\in A\}$ と表す．[@Nualart2018-Introduction], [@伊藤清1991] はいずれも丸括弧である．[@鎌谷20-モンテカルロ], [@Bain-Crisan2009-StochasticFiltering] では $\bP(-),\bE[-]$ を用いている．[@DelMoral-Penev2014] では $\bE(-),\bP(-)$ を用いる．]
...
X_-:\Om\to\R^T
$$
を **転置** と呼ぶ．^[筆者が考案した名称．族 $(X_t)_{t\in T}:T\to\L(\Om)$ としての見方と転置の関係になっているところから．[@伊藤清1991 p.232] は **見本過程（関数）** と呼び，記法 $X_\bullet$ を採用している．]
...
\Delta x(t):=x(t)-x(t-)
$$
で表す．^[[@Jacod-Shiryaev2003 p.325] に倣った．] ただし，$x(0-)=x(0)$ とする．^[[@Jacod-Shiryaev2003 p.3] に倣った．この結果，$\Delta x(0)=0$ であることに注意．]
