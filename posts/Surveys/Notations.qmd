...
* $\R_+$ で **非負実数** の全体，^[[@Jacod-Protter2012], [@LeGall2016], [@鎌谷20-モンテカルロ p.106], [@Helemskii2006 p.2] の記法に一致する．[@Evans2010 p.698] では同じ記法で正実数の全体を意味する．] $\R^+$ で **正実数** の全体がなす集合を表す：
$$
...
$$
を満たす関数 $f:\R^+\to\R$ の全体とする．^[[nLab](https://ncatlab.org/nlab/show/asymptotic+notation) に従った．$O$ は写像 $\R^\R\to P(\R^\R)$ を定める．[@Cramer1946 p.122], [@Jacod-Protter2012], [@DelMoral-Penev2014 p.xlvii] に一致．]
...
* $n$-次の実対称行列の全体を $S_n(\R)$ で表す．^[[@斎藤毅-線型代数 p.19] に一致する．[@Evans2010 p.697] では $\bS^n$ と表す．]
...
* 行列 $A\in M_{mn}(\bF)$ の転置を $A^\top$ で表し，^[[@吉田朋広2006-数理統計] などは転置を $A'$ で表す．[@斎藤毅-位相 p.86] では ${}^t\!A$ と表す．[@Evans2010 p.697] は $A^T$．] 共役転置を $A^*$ で表す．^[随伴行列ともいう [@斎藤毅-位相 p.87]．] $\bF=\bC$ の場合は $A^\top=A^*$．
...
* 行列 $A,B\in M_{mn}(\bC)$ の Hilbert-Schmidt 内積を^[[@Pedersen1989 p.119] は $(-|-)_{\text{tr}}$ で，[@Evans2010 p.697] は $A:B$ で表す．]
$$
\begin{align*}
    (B \mi A)_\HS&:=\Tr(A^*B)\\
...
$$
となり，$p=2$ の場合は $\abs{x}:=\norm{x}_2$ とも表す．^[[@Evans2010 p.699] に倣った．]
...
$$
と定める．^[[@Pedersen1989 p.51] 演習2.1.10 では $L(f)$，[@Dudley2002 p.390] 11.2節 では $\norm{-}_L$，[@Rudin1987 p.113] 演習11 では $M_f$，[@Evans2010 p.700] では $\Lip[f]$，[@Gilbarg-Trudinger2001 p.52] では $[f]_{1;T}$ と表している．]
...
* **発散** を
$$
\div u:=\Tr(Du)=\sum_{i=1}^n\pp{u}{x_i}
$$
で表す．^[[@Evans2010 p.703] に倣った．]

* $k=2$ かつ $m=1$ のとき，[**Hesse 行列**](https://ja.wikipedia.org/wiki/%E3%83%98%E3%83%83%E3%82%BB%E8%A1%8C%E5%88%97)
$$
D^2u=\begin{pmatrix}u_{x_1x_1}&\cdots&u_{x_1x_n}\\\vdots&\ddots&\vdots\\u_{x_nx_1}&\cdots&u_{x_nx_n}\end{pmatrix}
$$
...
\sgn(x):=2H(x)-1
$$
で定める．^[[@木田良才2020 p.131] 例12.21 に一致する．[@LeGall2016 p.161] では $\sgn=1_{(0,\infty)}-1_{\ocinterval{-\infty,0}}$，[@Evans2010 p.700] では $\sgn=1_{(0,\infty)}-1_{(-\infty,0)}$ と定めている（$0$での値が違う）．]
