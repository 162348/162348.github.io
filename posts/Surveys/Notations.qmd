...
$$
と表す．^[[@Dellacherie-Meyer1978], [@伊藤清1991-確率論] に倣った．]
...
$$
上への $\mu$ の制限を，$\mu|_A:\cA_\mu\cap A\to[0,\infty]$ で表す．^[[@Bogachev2007 p.23] に倣った．[@Dellacherie-Meyer1978] では $\mu|_A$ の定義域を $\cA|_A$ で表す．]
...
で表す．^[[@Nualart2018-Introduction p.1] に倣った．[@Gine-Nickl2021] ではイタリック体で $E$ と表している．[@DelMoral-Penev2014], [@Dellacherie-Meyer1978] では $\bE$ を用いる．]
...
* $S^1(E),M^1(E)$ などとイタリック体を用いた場合，$\cS^1(E),\cM^1(E)$ のうち Radon 測度のなす部分空間を表す．^[[@Dellacherie-Meyer1978] では，有界な Radon 測度の全体を $\cM_b^+(E)$ で表す．]
...
* 任意の関数空間 $\F(E)$ に対して，
  ...
  * 非負値のもののなす錐を $\F(E)_+:=\F(E;\R_+)$ で表す．^[[@Pedersen1989 p.222] に倣った．[@Dellacherie-Meyer1978] は $\F(E)^+$ で表す．]
